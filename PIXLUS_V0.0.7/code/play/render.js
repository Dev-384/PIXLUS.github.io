...
export default async function render(showIngameMenu=false){
	...
	if(showIngameMenu == true) await renderMenu("ingame");
}
...
async function renderLayer(currentWorld=0, worldLayerNumber=0){
	...
	for(let y = 0; y < yMax; y++){
		...
		for(let x = 0; x < xMax; x++){
			...
			blockPositionX = Math.floor(blockPositionX);
			let blockPositionY = (y * 16 ) - (PlayerPositions.Y ) + innerScreen.height / 2;
			blockPositionY = Math.floor(blockPositionY);
			...
			if(blockPositionX > -16 && blockPositionX < innerScreen.width + 16){
				...
				if(blockPositionY >  -16  && blockPositionY < innerScreen.height + 16){
					...
					if(
						...
						if(CachedCurrentBlock.aspects.standing.img?.animationLength > 0){
							let arrayOfNumberedFrames = [];
							for(let i = 0; i < CachedCurrentBlock.aspects.standing.img?.animationLength; i++){
						...
						image(
							`../../code/assets/blocks/images/${CachedCurrentBlock.aspects.standing.img.source}`,
						...
						if(CachedCurrentBlock.img?.animationLength > 0){
							let arrayOfNumberedFrames = [];
							for(let i = 0; i < CachedCurrentBlock.img?.animationLength; i++){
								arrayOfNumberedFrames.push(i);
							}
							currentBlockAnimationFrameNumber = getFrame(arrayOfNumberedFrames);
						}
						...
						image(
							`../../code/assets/blocks/images/${CachedCurrentBlock.img.source}`,
